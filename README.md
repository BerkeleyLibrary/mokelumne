...
| -------- | ------- | ----- |
| `AIRFLOW_UID` | Sets the `uid` the container runs as. | `AIRFLOW_UID="49003"` |
...
| `PYTHON_VERSION` | Sets the Python version used by the Airflow base image. | `PYTHON_VERSION="3.13"` | 
| `AIRFLOW_IMAGE_NAME` | Sets an alternate base image for Airflow, e.g. for `slim` images. Overrides both `AIRFLOW_VERSION` and `PYTHON_VERSION`. | `AIRFLOW_IMAGE_NAME="apache/airflow:slim-latest"` |
