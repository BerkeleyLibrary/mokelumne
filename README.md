...
|`AIRFLOW_CONN_AWS_DEFAULT`|Airflow AWS connection used by AWS hook. Provide AWS key/secret in `login`/`password`; add `region_name` and optional `aws_session_token` in `extra`.|`AIRFLOW_CONN_AWS_DEFAULT='{"conn_type":"aws","login":"AKIA...","password":"...","extra":{"region_name":"us-west-1","endpoint_url":"..."}}'`|
